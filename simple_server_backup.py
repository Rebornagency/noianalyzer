...
class DatabaseManager:
    def __init__(self):
        self.db_path = 'credit_management.db'
        self.init_db()
    ...
    def get_all_users(self):
        ...
        try:
            ...
            cursor.execute("SELECT email, credits, status, created_at FROM users ORDER BY created_at DESC")
            users = []
    ...
    def add_credits(self, email, credits):
        ...
        try:
            ...
            cursor.execute('''
                INSERT OR REPLACE INTO users (email, credits, status) 
                VALUES (?, COALESCE((SELECT credits FROM users WHERE email = ?), 0) + ?, COALESCE((SELECT status FROM users WHERE email = ?), 'active'))
            ''', (email, email, credits, email))
    ...
    def get_user_details(self, email):
        ...
        try:
            ...
            # Get user info
            cursor.execute("SELECT email, credits, status, created_at FROM users WHERE email = ?", (email,))
            ...
            return {
                'user': {
                    ...
                    'status': user_row[2] if user_row[2] else 'active',
    ...
    def get_system_stats(self):
        ...
        try:
            ...
            # For total credits purchased, we need to sum up all credits that have ever been added
            # This is a simplified approach - in a real system, you'd track this separately
            cursor.execute("SELECT SUM(credits) FROM users")
            result = cursor.fetchone()[0]
            stats['total_credits_purchased'] = result if result else 0
            
            # Calculate total credits used as the difference
            stats['total_credits_used'] = max(0, stats['total_credits_purchased'] - stats['total_outstanding_credits'])
    ...
    def verify_admin_key(self, provided_key):
        ...
        try:
            ...
            logger.error(f"Error in verify_admin_key: {str(e)}")
            return False
        ...
        try:
            ...
            try:
                ...
                if not result:
                    ...
                    cursor.execute("INSERT INTO users (email, credits, status) VALUES (?, ?, ?)", (email, max(0, credit_change), 'active'))
