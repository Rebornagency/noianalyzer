...
def display_credit_store():
    ...
    # Modern header styling - simplified to avoid raw HTML display
    ...
    for idx, package in enumerate(packages):
        with cols[idx % len(cols)]:
            ...
            card_html = f"""<div style="background: linear-gradient(145deg, #1a2436, #0f1722); border: 1px solid #2a3a50; border-radius: 16px; padding: 2rem; margin: 1.5rem 0; text-align: center; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; width: 100%; box-sizing: border-box; color: #FFFFFF; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Roboto', 'Helvetica Neue', Arial, sans-serif;"><h3 style="color: #FFFFFF; font-size: 1.8rem; font-weight: 700; margin: 0 0 1.5rem 0; padding-bottom: 1rem; border-bottom: 2px solid rgba(255, 255, 255, 0.1); width: 100%; text-align: center;">{("🌟 " + package["name"] + " (Popular)") if idx == 1 and len(packages) > 2 else package["name"]}</h3><div style="color: #FFFFFF; font-size: 1.3rem; font-weight: 600; margin: 1rem 0; width: 100%; text-align: center;">{package["credits"]} Credits</div><div style="color: #FFFFFF; font-size: 2.5rem; font-weight: 800; margin: 1rem 0; text-shadow: 0 2px 4px rgba(0, 0, 0, 0.3); width: 100%; text-align: center;">${package["price_dollars"]:.2f}</div><div style="color: #A0A0A0; font-size: 1rem; font-style: italic; margin: 0.5rem 0 1.5rem 0; width: 100%; text-align: center;">${package["per_credit_cost"]:.2f} per credit</div>"""
            ...
            # Add time savings section - simplified to avoid raw HTML display
            ...
            if not email:
                card_html += """<button disabled style="background: #6b7280; color: #FFFFFF; border: none; border-radius: 12px; padding: 1rem 1.5rem; font-size: 1.1rem; font-weight: 700; width: calc(100% - 2rem); cursor: not-allowed; margin: 0.5rem auto; display: block; text-align: center; height: auto; box-sizing: border-box;">Enter Email to Buy</button></div>"""
                ...
                if os.getenv('DEBUG_CREDITS', 'false').lower() == 'true':
                    st.markdown(f"""<script>console.info("[CREDITS] mounted");console.info("[CREDITS] pack render", "{package['package_id']}");console.info("[CREDITS] pack body type", typeof "{card_html[:50]}...", {{ length: "{len(card_html)}" }});</script>""", unsafe_allow_html=True)
