...
# ==============================
# UNIVERSAL LIGHT THEME HELPERS
# ==============================
# NOTE: These helpers complement (and will eventually replace) the legacy dark "Reborn" theme helpers above.
# They are intentionally appended at the *end* of the file to avoid breaking any imports that still rely on
# previously–defined functions.  Once the migration to the light theme is complete, the older helpers can be
# safely removed.

import math
from typing import List, Dict, Any, Tuple  # noqa: E402 – placed after Streamlit import purposely

# ------------------------------------------------------------------
# 1. Universal CSS loader (light theme)
# ------------------------------------------------------------------

def load_custom_css_universal(css_path: str = "static/css/reborn_theme.css") -> None:
    """Load the universal light-theme stylesheet and inject it into the Streamlit page.

    Args:
        css_path (str): Relative path to the compiled CSS file. Defaults to
            "static/css/reborn_theme.css" which already contains the light-theme
            definitions generated by the design spec.
    """
    try:
        with open(css_path, "r", encoding="utf-8") as f:
            st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)
        logger.info(f"✅ Loaded universal CSS from '{css_path}'")
    except FileNotFoundError:
        logger.warning(
            f"❌ Universal CSS file not found at '{css_path}'. Falling back to default Streamlit styles."
        )
    except Exception as exc:  # pragma: no cover – generic guard
        logger.error(f"Unexpected error while loading universal CSS: {exc}", exc_info=True)

# ------------------------------------------------------------------
# 2. Navigation bar with search icon (universal)
# ------------------------------------------------------------------

def render_navigation_with_search_universal() -> Tuple["st.delta_generator.DeltaGenerator", ...]:  # type: ignore
    """Render the 5-tab navigation bar plus decorative search icon.

    Returns:
        Tuple containing the five Streamlit tab objects – (Prior Month, Budget,
        Prior Year, Summary, NOI Coach) – in this exact order so that calling
        code can unpack them deterministically.
    """
    col_tabs, col_search = st.columns([6, 1])
    ...
    # Decorative search icon (no functional behaviour – purely visual)
    with col_search:
        st.markdown(
            """
            <div class="nav-search-container" tabindex="0">
                <svg class="nav-search-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
# ------------------------------------------------------------------
# 3. Formatting helpers for currency / percentage values with colour coding
# ------------------------------------------------------------------

def format_currency_value_universal(value: float) -> str:
    """Return value formatted as currency with monospaced font."""
    return (
        f'<span class="currency-value">${value:,.0f}</span>'
        if value is not None
        else ""
    )


def format_financial_change_universal(
    value: float,
    comparison_type: str = "change",
    *,
    is_percentage: bool = False,
) -> str:
    """Return formatted HTML span with colour-coded financial deltas.

    This function performs *presentation-only* formatting – the numeric value
    supplied is *not* altered in any way.
    """
    css_class: str
    prefix: str = ""
    suffix: str = "%" if is_percentage else ""
    ...
    if value > 0:
        ...
        prefix = "+" if is_percentage or not str(value).startswith("+") else ""
    elif value < 0:
        css_class = f"{comparison_type}-negative"
    else:
        css_class = f"{comparison_type}-neutral"

    formatted_val = f"{abs(value):.1f}" if is_percentage else f"{abs(value):,.0f}"
    return f'<span class="{css_class} {"percentage-value" if is_percentage else "currency-value"}">{prefix}{formatted_val}{suffix}</span>'

# ------------------------------------------------------------------
# 4. HTML table builders (one per comparison type)
# ------------------------------------------------------------------
...
def _build_table_html(
    ...
    *,
    headings: List[str],
    css_container_class: str,
    css_table_class: str,
    dollar_css_class: str,
    percent_css_class: str,
    comp_type: str,
) -> str:
    """Internal helper to DRY up repetitive table HTML construction."""
    header_html = "".join(f"<th>{h}</th>" for h in headings)
    body_rows = ""
    for row in data:
        body_cells = "".join(
            f"<td class='{css}'>{val}</td>" if css else f"<td>{val}</td>"
            for val, css in [
                (row["metric"], ""),
                (row["current"], ""),
                (row["compare"], ""),
                (row["delta_dollar"], dollar_css_class),
                (row["delta_percent"], percent_css_class),
            ]
        )
        body_rows += f"<tr>{body_cells}</tr>"

    return (
        f"<div class='{css_container_class}'>"
        f"<table class='{css_table_class}'>"
...
def create_prior_month_comparison_table_html(data: List[Dict[str, Any]]) -> str:
    headings = [
        "Metric",
        "Current",
        "Prior Month",
    ...
    return _build_table_html(
        data,
        headings=headings,
        css_container_class="prior-month-table-container",
        css_table_class="prior-month-table",
        dollar_css_class="change-dollar-column",
        percent_css_class="change-percent-column",
        comp_type="change",
    )


def create_budget_comparison_table_html(data: List[Dict[str, Any]]) -> str:
    headings = [
        "Metric",
        "Actual",
        "Budget",
    ...
    return _build_table_html(
        data,
        headings=headings,
        css_container_class="budget-comparison-table-container",
        css_table_class="budget-comparison-table",
        dollar_css_class="variance-dollar-column",
        percent_css_class="variance-percent-column",
        comp_type="variance",
    )


def create_prior_year_comparison_table_html(data: List[Dict[str, Any]]) -> str:
    headings = [
        "Metric",
        "Current Year",
        "Prior Year",
    ...
    return _build_table_html(
        data,
        headings=headings,
        css_container_class="prior-year-comparison-table-container",
        css_table_class="prior-year-comparison-table",
        dollar_css_class="growth-dollar-column",
        percent_css_class="growth-percent-column",
        comp_type="growth",
    )

# ------------------------------------------------------------------
# 5. Universal comparison view renderer
# ------------------------------------------------------------------

def render_comparison_view_universal(tab, comparison_type: str, data: List[Dict[str, Any]]):
    """Render a comparison table inside the supplied Streamlit tab.

    Args:
        tab: The Streamlit *tab* context returned from `st.tabs`.
        comparison_type (str): One of "prior_month", "budget", "prior_year".
        data (list[dict]): Pre-computed rows to render.  The exact structure is
            expected to match that produced by upstream calculation helpers.
    """
    # Map comparison type → builder function and heading text
    builders = {
        "prior_month": (
            create_prior_month_comparison_table_html,
            "## Current Month vs. Prior Month",
        ),
        "budget": (
            create_budget_comparison_table_html,
            "## Current Month vs. Budget",
        ),
        "prior_year": (
            create_prior_year_comparison_table_html,
            "## Current Month vs. Prior Year",
        ),
    }

    if comparison_type not in builders:
        logger.warning(f"render_comparison_view_universal: unknown comparison type '{comparison_type}'")
        return

    builder_fn, title_md = builders[comparison_type]

    with tab:
        st.markdown(title_md, unsafe_allow_html=True)
        table_html = builder_fn(data)
        st.markdown(table_html, unsafe_allow_html=True)

# ==============================
# END UNIVERSAL LIGHT THEME HELPERS
# ============================== 