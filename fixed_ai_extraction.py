...
def extract_noi_data(file: Any, document_type_hint: Optional[str] = None, 
    ...
    """
    logger.info(f"Extracting NOI data from {getattr(file, 'name', 'unknown')}")
    
    # Process the document using the single document function
    result = extract_single_document(file, document_type_hint)
    
    if result:
        logger.info(f"Successfully extracted data from {getattr(file, 'name', 'unknown')}")
        return result
    else:
        logger.warning(f"Failed to extract data from {getattr(file, 'name', 'unknown')}")
