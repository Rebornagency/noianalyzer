...
def display_insights(insights, property_name=""):
    """
    Display the financial insights in the Streamlit UI
    
    Args:
        insights: Structured insights dictionary containing financial insights
        property_name: Name of the property (optional)
    """
    if not insights:
        st.info("No insights available for this data set.")
        return
    ...
    if "summary" in insights and insights["summary"]:
        ...
        # Create a styled container for the executive summary
        st.markdown("""
        <div style="background-color: rgba(30, 41, 59, 0.8); padding: 20px; border-radius: 10px; 
                    border-left: 5px solid #4DB6AC; margin-bottom: 20px;">
            <h3 style="color: #4DB6AC; margin-top: 0;">Executive Summary</h3>
            <p style="color: #F0F0F0;">{summary}</p>
        </div>
        """.format(summary=executive_summary), unsafe_allow_html=True)
    ...
    if "insights" in insights and insights["insights"]:
        ...
        for i, insight in enumerate(key_insights[:6]):  # Limit to 6 key insights
            with cols[i % 2]:
                st.markdown(f"""
                <div class="insights-summary">
                    <h4 style="color: #4DB6AC; margin-top: 0;">{insight['title']}</h4>
                    <p style="color: #F0F0F0;">{insight['description']}</p>
                </div>
                """, unsafe_allow_html=True)
    
    # Opportunities and risks sections
    if "opportunities" in insights and insights["opportunities"]:
        with st.expander("Opportunities for Improvement", expanded=True):
            for item in insights["opportunities"]:
                st.markdown(f"""
                <div style="background-color: rgba(20, 31, 49, 0.7); padding: 15px; border-radius: 8px; 
                            border-left: 4px solid #22C55E; margin-bottom: 10px;">
                    <h4 style="color: #22C55E; margin-top: 0;">{item['title']}</h4>
                    <p style="color: #F0F0F0;">{item['description']}</p>
                </div>
                """, unsafe_allow_html=True)
    
    if "risks" in insights and insights["risks"]:
        with st.expander("Risk Factors", expanded=True):
            for item in insights["risks"]:
                st.markdown(f"""
                <div style="background-color: rgba(20, 31, 49, 0.7); padding: 15px; border-radius: 8px; 
                            border-left: 4px solid #EF4444; margin-bottom: 10px;">
                    <h4 style="color: #EF4444; margin-top: 0;">{item['title']}</h4>
                    <p style="color: #F0F0F0;">{item['description']}</p>
                </div>
                """, unsafe_allow_html=True)
    
    # Add a credit for AI generation
    st.caption("Insights generated by AI based on the financial data provided.")
