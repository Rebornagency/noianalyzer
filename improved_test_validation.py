"""
Improved test script to verify the financial content validation works correctly
"""
...
def create_test_files():
    ...
    # 1. Template file (no financial data) - truly empty values
    template_data = {
        'Category': [
            'Property:',
            'Period:',
    ...
    # 3. Truly empty file (just headers)
    empty_data = {
        'Category': [
            'Rental Income - Commercial',
            'Rental Income - Residential',
    ...
    return template_file, financial_file, empty_file

def test_validation():
    """Test the financial content validation"""
    print("Creating test files...")
    template_file, financial_file, empty_file = create_test_files()
    ...
    try:
        ...
        if 'combined_text' in template_result['content']:
            ...
            print(repr(template_result['content']['combined_text'][:200]))
        ...
        if 'combined_text' in financial_result['content']:
            ...
            print(repr(financial_result['content']['combined_text'][:200]))
        
        # Test empty file
        print(f"\n3. Testing empty file: {empty_file}")
        empty_result = preprocessor.preprocess(empty_file)
        empty_has_data = preprocessor.validate_financial_content(empty_result['content'])
        print(f"Empty file has financial data: {empty_has_data}")
        ...
        print(f"  Empty file correctly identified as lacking data: {not empty_has_data}")
        
        # Overall result - we want template and empty to be False, financial to be True
        success = (not template_has_data) and financial_has_data and (not empty_has_data)
        ...
        # Clean up
        for file in [template_file, financial_file, empty_file]:
