...
def format_narrative_for_display(narrative: str) -> str:
    ...
    narrative = re.sub(r'(\$?)(\d+(?:,\d+)*(?:\.\d+)?)\s*(?:million|m)\s*(?:dollars)?', 
                       lambda m: f"${float(m.group(2).replace(',', '')) * 1000000:,.2f}", 
                       narrative)
    
    # Find patterns like: $1.2k or $1,200 or 1,200 dollars
    narrative = re.sub(r'(\$?)(\d+(?:,\d+)*(?:\.\d+)?)\s*(?:thousand|k)\s*(?:dollars)?', 
                       lambda m: f"${float(m.group(2).replace(',', '')) * 1000:,.2f}", 
                       narrative)
    
    # Find any remaining number with dollar sign
    narrative = re.sub(r'\$\s*(\d+(?:,\d+)*(?:\.\d+)?)', 
                       lambda m: f"${float(m.group(1).replace(',', '')):,.2f}", 
                       narrative)
    
    # Find any percentages and format consistently
    narrative = re.sub(r'(\d+(?:\.\d+)?)(?:\s*)?(%|percent|percentage points?)',
